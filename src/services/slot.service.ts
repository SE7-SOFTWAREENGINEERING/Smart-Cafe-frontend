...
export const getSlots = async (_canteenName: string = 'Sopanam'): Promise<Slot[]> => {
    // Determine canteen ID or pass name directly if backend supports it
    // For now assuming backend might filter by canteen if we pass it, or we just get all live slots
    return getLiveSlots(new Date().toISOString().split('T')[0]);
};

export const bookSlot = async (slotTime: string, category: string, canteenName: string): Promise<void> => {
    try {
        const token = localStorage.getItem('token');
        // We need to find the slot ID for the given time first, or assume backend handles it by time?
        // The previous code passed time. Let's find the slot ID from live slots first.
        const slots = await getLiveSlots(new Date().toISOString().split('T')[0]);
        const slot = slots.find(s => s.time === slotTime);

        if (!slot) throw new Error('Slot not found');

        await axios.post(`${API_CONFIG.MAIN_BACKEND_URL}/bookings`,
            { slotId: slot.id, mealType: category, canteenName }, // Adjust payload to match backend expectation
